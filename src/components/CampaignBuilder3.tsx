...
export const CampaignBuilder3: React.FC<CampaignBuilder3Props> = ({ initialData }) => {
  ...
  // Only run once when first entering step 4, not on every render
  const [hasAutoGeneratedAds, setHasAutoGeneratedAds] = useState(false);
  
  useEffect(() => {
    // Only auto-generate if:
    // 1. We're on step 4
    // 2. No ads exist
    // 3. Keywords are available
    // 4. Not currently loading
    // 5. Haven't already auto-generated for this step
    if (currentStep === 4 && 
        campaignData.ads.length === 0 && 
        campaignData.selectedKeywords.length > 0 && 
        !loading && 
        !hasAutoGeneratedAds) {
      // Auto-generate ads when entering step 4 for the first time
      handleGenerateAds();
      setHasAutoGeneratedAds(true);
    } else if (currentStep !== 4) {
      // Reset flag when leaving step 4 so it can auto-generate again if user comes back
      setHasAutoGeneratedAds(false);
    }
